...
def _plot_comparacao_comercializacao(df):
    ...
    outliers = pd.DataFrame(columns=['tipo_de_comercializacao', 'valor', 'prod_und','nome_regiao'])  # DataFrame vazio com colunas
    ...
    for tipo, grupo in df.groupby('tipo_de_comercializacao'):
        ...
        top5 = top5[['tipo_de_comercializacao', 'valor', 'prod_und','nome_regiao']]  # ✨ Adiciona prod_und aqui
        outliers = pd.concat([outliers, top5])

    # Exibição dos resultados
    if not outliers.empty:
        st.subheader("Top 3 Outliers por Tipo de Comercialização")
        outliers = outliers.sort_values(by=['tipo_de_comercializacao', 'valor', 'prod_und','nome_regiao'], ascending=[True, False])
